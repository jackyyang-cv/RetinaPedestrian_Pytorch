...
def get_args():
    ...
    parser.add_argument('--save_step', type=int, default=1, help='Save every save_step epochs')
...
def main():
    ...
    train_path = os.path.join(data_path, 'retina-train-splitTrain.txt')#"train\\label.txt")#'train.txt')
    val_path = os.path.join(data_path, "retina-train-splitTest.txt")#"retina-train-splitTest.txt") #'retina-val.txt')##'val.txt')
    ...
    dataloader_train = DataLoader(dataset_train, num_workers=6, batch_size=args.batch, collate_fn=collater,shuffle=True)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.eval_step == 0:
            ...
            if (precision_global < precision):
                ...
                torch.save(retinaface.state_dict(), args.save_path + '/model_Best_epoch_{}.pt'.format(epoch + 1))
            print('Recall:',recall)
            print('Precision:',precision, "best Precision:", precision_global)
